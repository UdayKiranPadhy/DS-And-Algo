...
4 × 3 = 12
6 × 2 = 12
As you can see, calculations of 4 × 3 and 6 × 2 are not necessary. Therefore, we only 
need to consider factors up to √n because, if n is divisible by some number p, then 
n = p × q and since p ≤ q, we could derive that p ≤ √n.

Our total runtime has now improved to O(n1.5), which is slightly better. Is there a 
faster approach?
...
# Actual Solution https://leetcode.com/problems/count-primes/discuss/1403120/My-python-solutions
class Solution:
    def countPrimes(self, n: int) -> int:
        if n <= 2:
            return 0
