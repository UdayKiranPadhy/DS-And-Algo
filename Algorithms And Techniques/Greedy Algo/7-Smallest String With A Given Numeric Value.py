...
alphabet, so the numeric value of a is 1, the numeric value of b is 2, the numeric 
...
# Since we are forming the lexicographically smallest string we just simply fill our 
# result with as ( o˘◡˘o). But hold on, that result will not necessarily have the 
# required score c(ﾟ.ﾟ*) Ummm… Ok. Then we can add some zs to the back of the result 
# until the score reaches the required value (￢_￢;), so be it. Ofc if we are 
# missing less than 26 to the required score, we add something in between a and z.
...
model = Solution()
print(model.getSmallestString(5, 73))
