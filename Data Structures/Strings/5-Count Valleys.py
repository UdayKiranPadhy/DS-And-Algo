"""
Problem Statement: - https://www.hackerrank.com/challenges/counting-valleys/problem?h_l=interview&playlist_slugs%5B%5D=interview-preparation-kit&playlist_slugs%5B%5D=warmup
...
was an uphill,"U" , or a downhill, "D" step. Hikes always start and 
...
The hiker first enters a valley 2 units deep. Then they climb out and 
up onto a mountain 2 units high. Finally, the hiker returns to sea 
level and ends the hike.
...
if __name__ == "__main__":
    I = int
    R = range
    def listInput(): return [int(x) for x in input.strip().slipt(" ")]
    mod = 10000007
    inf = float('inf')
    for t in R(I(input())):
        steps = I(input())
